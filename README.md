...
* **Dynamic Action Masking:** Implements rigorous action masking (exactly 317 discrete actions) with a **One-Way Gate** selection system.
    * **Zero-Reboot Grade Stability:**
        * **Deterministic Loop Protection:** Differentiates between **Hard Progress** (changes in Floor, HP, Gold, or Deck Size) and **Screen Progress**. Action-use counters (the 10-use limit) are strictly preserved across screen transitions and only reset when Hard Progress is detected, physically preventing infinite menu loops (e.g., Shop/Cancel loops).
